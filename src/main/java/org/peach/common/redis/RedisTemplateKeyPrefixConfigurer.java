...
/**
 * 在容器初始化完成后，为 {@link StringRedisTemplate} 以及「Key 已为 {@link StringRedisSerializer}」的 {@link RedisTemplate} 安装
 * {@link RedisKeyPrefixStringRedisSerializer}，使经 Template 写入的 Key 自动带统一前缀。
 * <p>
 * 已为本序列化器或 null 的 KeySerializer 不会重复覆盖。
 * </p>
 * <p>
 * 由 {@link org.peach.common.redis.autoconfigure.PeachRedisKeyPrefixAutoConfiguration} 在存在 {@link RedisTemplate} 时注册为
 * {@link BeanPostProcessor}，勿再使用组件扫描注解，以免无 Redis 依赖时类加载失败。
 * </p>
 *
 * @author leiyangjun
 */
public class RedisTemplateKeyPrefixConfigurer implements BeanPostProcessor {
	...
	private static final RedisKeyPrefixStringRedisSerializer PREFIX_SERIALIZER =
		new RedisKeyPrefixStringRedisSerializer();
