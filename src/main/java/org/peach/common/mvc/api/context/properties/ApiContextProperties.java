 ...
 * <p>
 * 当 {@link #enabled} 为 {@code true} 时，未在<strong>控制器类</strong>上标注 {@code @AdminApi}/{@code @AppApi}/{@code @OpenApi} 时，整类使用默认 {@link #path} 对应的前缀；
 * 类上显式标注时以注解为准（不支持方法级形态注解）。{@code enabled=false} 时不注册路径前缀。
 * </p>
 *
 * @author leiyangjun
 */
...
public class ApiContextProperties {
	...
	/**
	 * 与全局前缀对应的路径段；仅在 {@link #enabled} 为 {@code true} 时生效。默认 {@link ApiType#ADMIN}。
	 * <p>
	 * {@code enabled=false} 时允许省略该配置项，不做「必须非空」校验。
	 * </p>
	 */
